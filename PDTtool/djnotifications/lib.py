...
def run_notification(notification):
  ...
  print "%s | %s" % (message,time_of_notification)
...
def clear_notifications(notification=None,
                        notification_publicid=None,
                        task_argument_keyword=None,
                        task_argument_value=None
                        ):
  if notification != None:
    #Get the notification listed.
    notifications = notification
  elif notification_publicid != None:
    #Get the notifications from the publicid.
    notifications = Notification.objects.filter(publicid=notification_publicid, deleted=False)
  elif task_argument_keyword != None and task_argument_value != None:
    #Get the notifications for the meeting.
    notifications = Notification.objects.filter(
                                     task_argument__argument_keyword_name=task_argument_keyword,
                                     task_argument__argument_keyword_value=task_argument_value,
                                     deleted=False)
  else:
...
def send_html_email(subject,text_message,html_message,from_email,to_email):
