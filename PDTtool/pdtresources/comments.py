...
def comment_form(request, commented_object = 'topic', commented_object_id=None):
	...
	action_url = "/viewtopic/?publicid=%s"%(request.GET['publicid'])
	...
	if commented_object == 'topic':
		...
		addfeedback+= '<a href="javascript:void(0);" class="feedback" onclick="$(\'#%s\').toggle();">Comment</a>'%form_div_id
	...
	content+=		'<button type="submit" class="btn btn-primary light-button">Comment</button>'
				#Close the form.
	content+=	'</form>'
...
def recursive_comments(request, parentcommentid, top_comment=True):
	...
	comment_attr = 'comment-publicid="%s"'% (parentcomment.publicid)

	#Define the html content of the comment
	content = '<div class="comment_content border-radius-5">'
	...
	content+=	'<a href="javascript:void(0)" class="btn btn-small reply_button light-button" style="margin-top: -4px;" onclick="$(\'#comment_comment_form_%s\').toggle();">reply</a>'% parentcomment.publicid
	...
	#Create the comments.
	return '<div class="comment_content_wrapper %s" %s>%s<div class="clear-fix"></div></div><!-- .%s -->'% (commentclass, comment_attr, content,commentclass) 