...
def viewusers(request):
	...
	if request.method == 'POST' and ('edit_user' in request.POST or 'add_user' in request.POST):
		...
		if 'edit_user' in request.POST:
			...
			extendeduser = um_models.ExtendedUser.objects.get(publicid=publicid)
		else:
			extendeduser = False
		...
		if not check_existing_user(newusername) or extendeduser and extendeduser.user.username == newusername:
			...
			if extendeduser:
				...
				extendeduser = um_models.ExtendedUser(user=user,phonenumber=newphonenumber)
			...
			#If there is no notifications object, create one.
			if not hasattr(extendeduser,'usernotifications'):
				usernotifications = um_models.UserNotifications(extendeduser=extendeduser)
	...
	if request.method == 'GET' and 'search' in request.GET:
		...
		users_list = um_models.ExtendedUser.objects.filter(
		...
		users_list = um_models.ExtendedUser.objects.exclude(user__is_superuser=True)
	...
	users = [{
		...
		'extendeduser':u,
		'user':u.user,
		'user_form':UserManagementForm(returnfunc(request, u)).as_table(),
	...
	add_user_form = UserManagementForm(request.POST if 'add_user' in request.POST else None).as_table()
