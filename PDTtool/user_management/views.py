		...
		if user:
			...
			if user.is_active:
				...
				if user.is_superuser:
					...
					if um_models.ExtendedUser.objects.filter(user=user).count() == 0:
						#Create a temporary phone number.
						extendeduser = um_models.ExtendedUser(user=user,phonenumber="000000000")
...
def register(request):
	...
	if request.method == 'POST':
		...
		###
		registerform = um_forms.RegisterForm(formdata)
		...
		if registerform.is_valid():
			...
			extuser = um_models.ExtendedUser(user=newuser,phonenumber=formdata['phonenumber'])
			extuser.save()

			notifications = um_models.UserNotifications(extendeduser=extuser)
			notifications.save()
...
def activate(request):
	...
	if 'userid' in request.GET:
		...
		try:
			...
			publicid = um_models.ActivateUserDB.objects.get(publicid=publicid_string)
			#Get the publicid database field
			extendeduser = um_models.ExtendedUser.objects.get(publicid=extendeduser_publicid)
		except:
...
def forgotpassword(request):
	...
	if 'publicid' in request.GET and 'userid' in request.GET:
		...
		try:
			#Get the publicid database field
			publicid = um_models.ForgotPasswordDB.objects.get(publicid=publicid_string)
			#Get the publicid database field
			extendeduser = um_models.ExtendedUser.objects.get(publicid=extendeduser_publicid)
		except:
			#publicid doesn't exist anymore
	...
	if request.method == 'POST':
		...
		forgotpassword = um_forms.ForgotPassword(request.POST)
		...
		forgotpassword = um_forms.ForgotPassword()
...
def accountsettings(request):
	...
	extendeduser = um_models.ExtendedUser.objects.get(user=request.user)
	...
	if request.method == 'POST':
		...
		accountsettingsform = um_forms.AccountSettingsForm(request.POST)
		...
		accountsettingsform = um_forms.AccountSettingsForm(user_form_contents)
...
########
# These are the notifications, the panel to turn off notifications.
#	-The view file is user_management/notifications.html 
########
@login_required
def notifications(request):

	extendeduser = um_models.ExtendedUser.objects.filter(user=request.user)

	if extendeduser.count() > 0:

		extendeduser = extendeduser[0]

	if request.POST:
		notification_dialogs = um_forms.UserNotificationsForm(request.POST,instance=extendeduser.usernotifications)
		notification_dialogs.save()
	else:
		notification_dialogs = um_forms.UserNotificationsForm(instance=extendeduser.usernotifications)

	context = {
		'title':'Manage Notifications',
		'notification_dialogs':notification_dialogs,
		'notification_sets':settings.NOTIFICATION_SETS,
		'notification_delimeter':settings.NOTIFICATION_DELIMETER
	}
...
def resetpassword(request):
	...
	#Add the reset form.
	context['ResetPasswordForm'] = um_forms.ResetPasswordForm()
