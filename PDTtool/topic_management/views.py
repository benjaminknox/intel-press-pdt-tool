...
from meeting_management import resources
...
from topic_management.resources import generate_topic_slug, get_topic
...
def viewtopics(request):
	...
	if request.method == 'POST':
		...
		if topicform.is_valid():
			...
			#Thesea re the names of the files.
			for name,f in files.iteritems():
	...
	if 'mytopics' in request.GET:
		...
		topics = topics.filter(user=request.user).order_by('readyforreview', '-supervisor_released')
	else:
		#Update the 
		topics = topics.order_by('-readyforreview', 'supervisor_released')
...
def viewtopic(request):
		...
		if topic_object.user == request.user or request.user.groups.filter(Q(name='Supervisor')).count() != 0:
			...
			if (
				...
				if 'topic_presentationlength' in request.POST:
					...
					#Get the preferred meeting publicid.
					preferredmeeting_publicid = request.POST['preferredmeeting_publicid']
					...
					preferredmeeting_publicid = 'Not Selected'

					#Output the schedule resources.
					scheduleresources.remove_from_schedule(topic_object)
				...
				if preferredmeeting_publicid != 'Not Selected':

					#This is id of the meeting that the user would like to be in.
					scheduleresources.automatic_scheduler(topic=topic_object.publicid,
																								preferredmeeting=preferredmeeting_publicid)
	...
	topic = get_topic(request,topic_object)
	...
	documents = [
		...
		for document in topic_object.documents.all().order_by('pk')
	]
	...
	#Get the meetings available
	meetings_available = resources.get_available_meetings()
	...
	context= {
		...
		'meetings_available':meetings_available
	}
